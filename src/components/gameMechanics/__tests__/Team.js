...
test.each([
  ['add', new Team, [new Bowerman("Oleg")], [new Bowerman("Oleg")]],
  ['addAll', new Team, [new Bowerman("Oleg"), new Swordsman("Misha"), new Magician("Ivan")], [new Bowerman("Oleg"), new Swordsman("Misha"), new Magician("Ivan")]],
])('1', (targetFunction, team, characters, expected) => {
  team[targetFunction](...characters);
  expect(team.toArray()).toEqual(expected);
});

test('2', () => {
  let team = new Team();
  let character = new Bowerman("Oleg");
  team.add(character);
...
test('3', () => {
  let team = new Team();
  let characters = [new Bowerman("Oleg"), new Swordsman("Misha"), new Magician("Ivan")];
  team.addAll(...characters, ...characters);
